...
Control-plane clients authenticate with unique, scoped `actx1` HMAC proofs.
`AUTOCONTEXT_SERVER_TOKEN` is a compatibility signing key of at least 32 bytes,
not a bearer credential; the TypeScript TUI reads it locally and signs each
HTTP request or WebSocket reconnect. Proofs last at most 60 seconds and bind the
method, raw path and query, Origin, audience, capabilities, and one-time JTI.
HTTP sends `Authorization: Bearer actx1...`; WebSocket offers the exact
`actx1...` proof as its subprotocol. Raw secrets, URL userinfo, and credential
query parameters are rejected. See [Control-plane authentication](control-plane-auth.md)
before building another client or exposing a shared deployment.
...
| `AUTOCONTEXT_SERVER_TOKEN`                                                       | Compatibility HMAC signing key of at least 32 bytes. It configures the implicit `env` credential; never send the value as a bearer token. Clients derive a fresh, scoped `actx1` proof for each request or WebSocket reconnect.                                                                                                                                                                                                                                                                                                                                            |
| `AUTOCONTEXT_SERVER_CREDENTIALS_FILE`                                            | Absolute path to a version 1 scoped-credential registry. On POSIX it must be a same-owner regular file with mode `0400` or `0600`, no symlink components, and sorted unique capability arrays. Rejected on Windows until owner and DACL validation is available; use `AUTOCONTEXT_SERVER_TOKEN` there. See [Control-plane authentication](control-plane-auth.md).                                                                                                                                               |
| `AUTOCONTEXT_ALLOW_TOKENLESS_LOOPBACK`                                           | Set to `1` only for an explicitly accepted local-development exception. It never permits a non-loopback peer and must not be used behind a reverse proxy.                                                                                                                                                                                                                                                                                                                                                |
...
| `AUTOCONTEXT_LOCAL_MODEL`                                                        | One model id for every otherwise-unset role/tier slot on non-Anthropic providers. Explicit `AUTOCONTEXT_MODEL_<ROLE>` and `AUTOCONTEXT_TIER_<TIER>_MODEL` values win. With routing `off`, known single defaults include `ollama=llama3.1`, `openai`/`openai-compatible=gpt-5.6-terra`, `openrouter=anthropic/claude-sonnet-5`, and `vllm=default`. With routing `auto`, OpenAI/openai-compatible use Sol/Terra/Luna and OpenRouter uses Opus/Sonnet/Haiku for frontier/mid-tier/fast. Set this variable for a generic OpenAI-compatible endpoint that serves one model. |
