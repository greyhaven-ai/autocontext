...
| `AUTOCONTEXT_LEAN_HIDDEN_CONTEXT_BUDGET_TOKENS`                      | Hidden/implicit context budget for lean surfaces (default: `0`)                                     |
| `AUTOCONTEXT_LEAN_TOOL_ALLOWLIST`                                    | Comma-separated minimal tool surface advertised by the lean profile                                 |
...
Set `AUTOCONTEXT_HARNESS_PROFILE=lean` when an external agent should use autocontext more like Pi: the resolved runtime profile caps prompt assembly to `AUTOCONTEXT_LEAN_CONTEXT_BUDGET_TOKENS`, keeps hidden context at zero by default, and exposes a small comma-separated tool allowlist for agent surfaces that enforce tool affordances.
