...
Control-plane servers bind to loopback by default. Any non-loopback bind
requires a random `AUTOCONTEXT_SERVER_TOKEN` of at least 32 characters. HTTP
clients send `Authorization: Bearer <token>`; browser WebSocket clients offer
the `autocontext.bearer.<base64url-token>` subprotocol. The TypeScript TUI reads
the token from its environment. URL userinfo and credential-bearing query
parameters are rejected. This is single-tenant containment, so use TLS plus an
authenticated and authorizing reverse proxy for shared deployments.
...
| `AUTOCONTEXT_SERVER_TOKEN`                                                       | Random control-plane token of at least 32 characters. Required for non-loopback HTTP/WebSocket binds and recommended behind a loopback reverse proxy. Clients use a bearer header or the browser-compatible `autocontext.bearer.<base64url-token>` WebSocket subprotocol; URL credentials are rejected. This shared token is single-tenant containment, not scoped multi-user authorization.                                                                                                                                                                                  |
