...
`AUTOCONTEXT_HARNESS_PROFILE=lean` resolves a Pi-shaped runtime profile. The core generation loop enforces the prompt context cap from `AUTOCONTEXT_LEAN_CONTEXT_BUDGET_TOKENS`; hidden/implicit context and the lean tool allowlist are exported as profile metadata for agent surfaces that enforce those affordances.
