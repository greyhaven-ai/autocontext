...
AutoContext supports local model distillation via [MLX](https://github.com/ml-explore/mlx). Training requires **macOS with Apple Silicon** (M1/M2/M3/M4) — MLX uses Metal directly and cannot run inside Docker or Linux VMs.
...
cd autocontext
uv sync --group dev --extra mlx
uv run autoctx train --scenario grid_ctf --data training/data.jsonl --time-budget 60
```

This produces a safetensors checkpoint loadable by `MLXProvider` for local inference.

For sandboxed OpenClaw agents that need to trigger host-side training, see the [Host-Training Bridge](../docs/mlx-training.md) documentation, which covers file-based launchd triggers, gateway exec, and HTTP bridge options.

Full setup guide: [docs/mlx-training.md](../docs/mlx-training.md)
