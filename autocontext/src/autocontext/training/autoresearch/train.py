...
def _build_torch_model(cfg: ModelConfig, torch_module: Any) -> Any:
...
def _torch_peak_memory_mb(torch_module: Any, device: Any) -> float:
    try:
        ...
        logger.debug("training.autoresearch.train: suppressed torch memory read", exc_info=True)
        return 0.0


def _save_torch_inference_bundle(
    *,
    model: Any,
    cfg: ModelConfig,
    tokenizer: Any,
    ...
    *,
    ...
    for i in range(n_samples):
        try:
            ...
            logger.debug("training.autoresearch.train: suppressed CUDA assessment error", exc_info=True)
...
def _run_cuda_training(
    *,
    ...
    try:
        ...
        logger.debug("training.autoresearch.train: suppressed torch memory reset", exc_info=True)
