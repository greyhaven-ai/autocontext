...
from autocontext.scenarios.custom.workflow_codegen import generate_workflow_class
from autocontext.scenarios.custom.workflow_designer import design_workflow
...
class WorkflowCreator:
    ...
    def create(self, description: str, name: str) -> ScenarioInterface:
        spec = design_workflow(description, self.llm_fn)
        errors = validate_for_family("workflow", asdict(spec))
        if errors:
            raise ValueError(f"workflow spec validation failed: {'; '.join(errors)}")

        custom_dir = self.knowledge_root / CUSTOM_SCENARIOS_DIR
        scenario_dir = custom_dir / name
        scenario_dir.mkdir(parents=True, exist_ok=True)

        source = generate_workflow_class(spec, name=name)
        source_errors = validate_source_for_family("workflow", source)
        if source_errors:
            raise ValueError(f"workflow source validation failed: {'; '.join(source_errors)}")
        ...
        (scenario_dir / "spec.json").write_text(
            json.dumps(
                {
                    "name": name,
                    "scenario_type": get_family_marker("workflow"),
        ...
        (scenario_dir / "scenario_type.txt").write_text(
            get_family_marker("workflow"),
            encoding="utf-8",
        )

        cls = load_custom_scenario(custom_dir, name, WorkflowInterface)
        from autocontext.scenarios import SCENARIO_REGISTRY

        SCENARIO_REGISTRY[name] = cls
        logger.info("registered workflow scenario '%s'", name)
