...
from autocontext.storage.artifact_snapshots import SnapshotMethods
...
class ArtifactStore(
    ...
    ArtifactGenerationPersistenceMethods,
    ArtifactWriteMethods,
    HarnessCodegenMethods,
    SkillLessonMethods,
    SnapshotMethods,
):
    def __init__(
    ...
    def read_skills(self, scenario_name: str) -> str:
        ...
        active_lines = [line for line in section.splitlines() if "<!-- autocontext:lesson-status=stale -->" not in line]
    ...
    def _list_python_modules(directory: Path) -> list[str]:
        ...
        if not directory.exists():
            return []
        return sorted(path.stem for path in directory.glob("*.py") if path.is_file() and not path.name.startswith("_"))
