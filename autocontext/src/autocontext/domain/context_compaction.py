"""Deterministic context compaction shared by knowledge and persistence.

This neutral leaf keeps structured context bounded before the final prompt budget
...
def _compact_markdown(
    ...
    *,
    ...
    compacted_sections = [_compact_section(section, prefer_recent=prefer_recent) for section in selected_sections]
    compacted = "\n\n".join(section for section in compacted_sections if section.strip()).strip()
    if compacted and compacted != text:
...
def _compact_table(text: str, *, max_tokens: int) -> str:
    ...
    compacted_trailing_context = _compact_markdown(trailing_context, max_tokens=max_tokens) if trailing_context else ""
    compacted_lines = [
        *pre_table_lines[:4],
        *table_header,
        *selected_rows,
...
def _is_structured_line(line: str) -> bool:
    return line.startswith(("- ", "* ", "> ")) or bool(re.match(r"^\d+\.\s+", line)) or ":" in line
