"""Neutral hint contracts and volume control (AC-340, AC-946).
...
class HintManager:
    ...
    def from_dict(
        ...
        *,
        ...
        active = [RankedHint.from_dict(item) for item in data.get("active", []) if isinstance(item, dict)]
        archived = [RankedHint.from_dict(item) for item in data.get("archived", []) if isinstance(item, dict)]
