...
class CampaignPlan(StrictModel):
    ...
    def _validate_job_identities(self) -> CampaignPlan:
        ...
        cohort_lanes: dict[str, tuple[str, str, int, str, str]] = {}
...
class ScenarioCampaignWorker:
    ...
    def execute(self, assignment: CampaignAssignment) -> CampaignJobResult:
        ...
        write_json(output_path, _execution_output_dict(output))
        ...
        CampaignSchedulerRuntimePlan(
            ...
            jobs=tuple(
                _job_request(
                    plan,
                    item,
                    resources=_job_resources(settings),
        ...
        workers=(
            CampaignWorkerBinding(
                descriptor=WorkerDescriptor(
                    worker_id=f"{settings.executor_mode}-primary",
                    runtime=settings.executor_mode,
                    resources=_worker_resources(settings, plan.max_concurrency),
                    ...
                    environment_labels={"executor_mode": settings.executor_mode},
                ),
...
def _job_request(
    ...
    *,
    ...
    return CampaignJobRequest(
        ...
        required_capabilities=frozenset({"scenario_evaluation"}),
        reservation=_campaign_job_reservation(item),
        ...
        lane = lanes.setdefault(
            item.lane_id,
            {
                ...
                "verifier_contract_ref": (
                    evaluation_identity.verifier_contract_ref
                    if evaluation_identity is not None
                    else item.verifier_contract_ref
                ),
                "seeds": [],
                "holdout_refs": [],
                "weight": 1.0,
...
def _campaign_job_reservation(item: CampaignPlanJob) -> SchedulerBudget:
    ...
    return SchedulerBudget(
        ...
        compute_units=declared.compute_units,
        jobs=max(declared.jobs, 1),
        shared_evidence_tokens=declared.shared_evidence_tokens,
    )


def _job_resources(settings: AppSettings) -> SchedulerResources:
...
def _worker_resources(settings: AppSettings, concurrency: int) -> SchedulerResources:
    per_job = _job_resources(settings)
    return SchedulerResources(
...
def _execution_output_dict(output: ExecutionOutput) -> dict[str, Any]:
    return {
...
def _validate_campaign_evaluation_identity(
    ...
    for item in plan.jobs:
        if item.evaluator_epoch != identity.evaluator_epoch:
            raise ValueError(
                f"campaign job {item.job_id!r} evaluator_epoch does not match the active scenario evaluator"
            )
        if item.verifier_contract_ref != identity.verifier_contract_ref:
            raise ValueError(
                f"campaign job {item.job_id!r} verifier_contract_ref does not match the runtime verifier contract"
            )
