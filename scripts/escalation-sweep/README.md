...
- `autoctx` CLI installed (either a published release `pip install autocontext==0.18.0`
...
python scripts/escalation-sweep/fetch_manifest.py .sweep/0.17.0/manifest.json

# 2. Run the sweep. One solve per scenario, 2 generations each by default.
bash scripts/escalation-sweep/run_sweep.sh \
    .sweep/0.17.0/manifest.json \
    .sweep/0.17.0/results \
    --iterations 2 --timeout 600

# 3. Classify + tally.
python scripts/escalation-sweep/summarize.py .sweep/0.17.0/results
```
