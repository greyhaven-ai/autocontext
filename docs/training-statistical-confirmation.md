...
Each `TrainingPromotionArtifact` contains the raw incumbent/challenger trials, evaluator epoch, verifier digest, cohort, fixtures, seeds, scores, validity and dimension observations, stopping rationale, confidence interval, and total evaluation cost. Passing the stored trials and protocol back to `evaluate_training_promotion` reproduces the decision. Replay rejects confirmation or held-out evidence beyond the configured pair counts, so surplus post-hoc observations cannot change a promotion.
