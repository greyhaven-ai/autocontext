...
export class LLMJudge {
    ...
    async evaluate(opts) {
        let systemPrompt = "You are an expert judge evaluating an AI agent's output. " +
        ...
        for (let s = 0; s < this.samples; s++) {
            ...
            for (let attempt = 0; attempt < 2; attempt++) {
                ...
                if (score > 0 || !reasoning.includes("Failed to parse"))
                    break;
            }
            scores.push(score);
    ...
    buildJudgePrompt(opts) {
        ...
        if (opts.calibrationExamples?.length) {
            ...
            for (let i = 0; i < opts.calibrationExamples.length; i++) {
                ...
                lines.push(`**Example ${i + 1}** — Score: ${score}\n` +
                    `Human notes: ${notes}\n` +
