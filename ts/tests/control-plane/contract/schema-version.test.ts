...
function v(value: string): SchemaVersion {
...
describe("compareSchemaVersions", () => {
  test("compares major first then minor", () => {
    ...
    expect(compareSchemaVersions(v("1.0"), v("1.1"))).toBeLessThan(0);
    expect(compareSchemaVersions(v("1.2"), v("1.1"))).toBeGreaterThan(0);
    expect(compareSchemaVersions(v("1.99"), v("2.0"))).toBeLessThan(0);
    expect(compareSchemaVersions(v("10.0"), v("2.99"))).toBeGreaterThan(0);
  });
});

describe("isReadCompatible (semver-lite: same MAJOR)", () => {
  test("true when MAJOR matches regardless of MINOR direction", () => {
    expect(isReadCompatible(v("1.0"), v("1.0"))).toBe(true);
    expect(isReadCompatible(v("1.0"), v("1.5"))).toBe(true);
    expect(isReadCompatible(v("1.5"), v("1.0"))).toBe(true);
  });

  test("false when MAJOR differs", () => {
    expect(isReadCompatible(v("1.0"), v("2.0"))).toBe(false);
    expect(isReadCompatible(v("2.0"), v("1.0"))).toBe(false);
  });
});

describe("canWriteVersion (tools must not downgrade)", () => {
  test("consumer can write a version >= declared repo version", () => {
    expect(canWriteVersion(v("1.0"), v("1.0"))).toBe(true);
    expect(canWriteVersion(v("1.1"), v("1.0"))).toBe(true); // writing 1.0 is OK when 1.1 consumer
  });
  ...
  test("consumer CANNOT write a lower version than declared", () => {
    ...
    expect(canWriteVersion(v("1.0"), v("1.5"))).toBe(false);
    expect(canWriteVersion(v("1.0"), v("2.0"))).toBe(false);
  });
});
