 ...
 */
const defaultExecutor: TrainingExecutor = async (_config, checkpointDir) => {
  writeFileSync(
    join(checkpointDir, "checkpoint_info.json"),
    JSON.stringify({
      backend: _config.backend,
      trainingMode: _config.trainingMode,
      baseModel: _config.baseModel,
      status: "trained",
  ...
  return { success: true, metrics: { epochs: _config.maxEpochs ?? 3 } };
};
