 ...
 *   node scripts/sync-python-harness-optimization-schemas.mjs --check  # CI: drift-check only
 *
...
const SRC_DIR = join(TS_ROOT, "src/harness-optimization/contract/json-schemas");
const DST_SCHEMAS_DIR = join(PY_ROOT, "src/autocontext/harness_optimization/contract/json_schemas");
...
const OUR_BANNER = `# AUTO-GENERATED from ts/src/harness-optimization/contract/json-schemas/ — DO NOT EDIT.
...
if (checkOnly) {
  if (drift) {
    console.error("Python harness-optimization sync has drift:");
    for (const a of actions) console.error("  " + a);
    console.error("Run: node scripts/sync-python-harness-optimization-schemas.mjs");
    process.exit(1);
  }
  console.log("Python harness-optimization schemas + models are up to date.");
  process.exit(0);
}

for (const a of actions) console.log(a);
if (actions.length === 0) console.log("Python harness-optimization sync unchanged.");
