#!/usr/bin/env node
/**
 * Regenerate ts/src/harness-optimization/contract/generated-types.ts from the
 * canonical JSON Schemas under ts/src/harness-optimization/contract/json-schemas/.
 *
 * Usage:
 *   node scripts/generate-harness-optimization-types.mjs           # write file
 *   node scripts/generate-harness-optimization-types.mjs --check   # diff-only (CI)
 *
...
const SCHEMAS_DIR = join(TS_ROOT, "src/harness-optimization/contract/json-schemas");
const OUTPUT_FILE = join(TS_ROOT, "src/harness-optimization/contract/generated-types.ts");
...
if (args.includes("--check")) {
  ...
  if (existing !== final) {
    ...
    console.error("run: node scripts/generate-harness-optimization-types.mjs");
    process.exit(1);
  }
