...
- AC-767: authoritative ground-truth fixture loader. Drop a JSON manifest at `autocontext/knowledge/<scenario>/fixtures.json` listing `(key, source, expected_sha256?)` triples, set `AUTOCONTEXT_FIXTURE_LOADER_ENABLED=true`, and the generation-1 preflight stage will pre-fetch each entry (cache under `.fixture-cache`, checksum-verify on every read, refetch on source-URL change). The rendered fixture summary lands in `ctx.fixtures_section` and is threaded through the prompt-budget pipeline (`prompts/context_budget.py` `fixtures` component with an 800-token cap) into the competitor / analyst / coach / architect prompts, so agents actually see the authoritative reference data. The raw bytes are also available programmatically via `ctx.fixtures[key].bytes_`. Default fetcher (`UrlFetcher`) handles `http(s)://`, `file://`, and bare local paths; unknown schemes wrap as `FixtureFetchError`. Cache keys and scenario names are validated against `^[A-Za-z0-9_][A-Za-z0-9_.\-]*$` so manifest values can't escape the cache root. Missing manifest is a graceful no-op (no error, no event). See `autocontext/docs/fixture-loader.md` for the manifest format and the end-to-end flow.
