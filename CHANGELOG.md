...
- Kernel promotion now names strict-FP32 and relaxed-precision profiles, binds
  ...
  gates, and uses a persisted Bonferroni proposal budget. The H100 campaign
  writes profile-namespaced evidence from disjoint primary/confirmation plans
  (AC-994).
...
- TypeScript candidate promotion can now run through a durable, idempotent activation transaction that stages actuator application, validates and activates a private component graph, drains the prior runtime, cuts over the observed runtime and atomic pointer, then disposes the old graph. Failed activation restores the prior runtime; shadow/canary deployments retain the baseline pointer and inherit fail-closed effect policy; restart recovery, stale-pointer repair, live rollback, registry promotion integration, and sanitized audit events are included (AC-961).
- TypeScript runtime composition now exposes deterministic observable snapshots across component/provider identities, scoped hooks/tools/grants, subscriptions, timers, tasks, temporary resources, active pointers, blocked capabilities, and lifecycle errors. A seeded bounded scheduler and property/fault harness compare dynamic histories with clean boot, inject every activation/disposal/cutover failure and interruption boundary, assert leak-free quiescence, compare compensatable effects by declared equivalence, and explicitly exclude irreversible emissions from exact equality; a separate longer sweep is available for non-default CI (AC-962).
...
- `AUTOCONTEXT_OFFLINE=1` enforces that the engine never initiates an outbound connection. Scoped by who initiates: engine-initiated egress (providers, webhooks, fixture downloads, endpoint probes) fails closed with an error naming what was blocked, while operator-initiated access such as SSH is out of scope, so an airgapped host does not have to be unreachable. The CLI runtimes (`claude-cli`, `codex`, `pi`, `pi-rpc`, `hermes`) refuse to start, because they shell out to binaries that make their own network calls and no guarantee can cover another program's sockets; local endpoints (`ollama`, `vllm`, `mlx`) are unaffected. Configuring offline mode alongside egress settings is a startup error rather than a silent precedence rule. Enforcement is verified by a test that runs a complete generation with a socket-level guard and asserts zero connection attempts, plus a CI guard that fails on new unguarded egress call sites (AC-917).
- Preflight now reports when a configured dollar budget can never be reached because every role on the run routes to a locally hosted endpoint priced at zero, and points at `AUTOCONTEXT_GENERATION_TIME_BUDGET_SECONDS` as a bound that does apply. Advisory rather than blocking: the configuration is inert rather than wrong. The gates themselves still measure dollars, deliberately -- a dollar setting silently bounding wall-clock or tokens would be a limit in a unit its name does not say (AC-934).

### Fixed

- Provider credential resolution now matches keys to transports across judge, default-agent, and per-role paths. OpenRouter no longer falls through to `OPENAI_API_KEY` (and is supported as a named default agent provider), mixed-provider roles no longer inherit another provider's generic key, and keyless vLLM/Ollama paths cannot receive an Anthropic credential (AC-933).
...
- The provider capability probe no longer reports thinking-path structured output that a provider does not actually honor. Support is now declared by the provider rather than inferred from a method signature, which cannot distinguish a parameter that is honored from one that is accepted and discarded (AC-936).
...
- The Python interactive server now honors the base `safe_run_stop_v1` capability and advertises it in its hello frame when a run manager is wired: `stop` is cooperative (it takes effect at the top-of-generation boundary and wakes a paused run), run-scoped (a `client_run_id` that does not match the active run is rejected, so a stop cannot target another run; a run started without a client id is addressable by the server run id), and live-idempotent by `command_id` (a duplicate or reconnected stop re-acknowledges without re-triggering while the run is active). Stop versus natural completion is deterministic (first terminal outcome wins, arbitrated by a SQLite compare-and-set), a stopped run is terminal (it cannot be resumed and is a terminal watch state), completed per-generation results are preserved (pending writes still flush) and the terminal receipt counts durable progress, and the loop emits one correlated `run_stopped` receipt with `reason: operator`. The capability is decoupled from the transcript: durable reconnect-after-terminal replay of the ack and receipt still requires `run_transcript_v1` and remains TypeScript-only (AC-894).
...
- Removed the unused per-file opt-out LLM prompt (spec site 2) and stale Layer 8 TODO markers from the instrument PR-body pipeline; only the two implemented enhancement sites (per-call-site rationale and session summary) remain wired (AC-859).

## [0.12.0] - 2026-07-19

### Added

- TypeScript interactive transcripts now advertise `safe_run_stop_v1`: a required-ID, run-bound stop command wakes paused runs, takes effect at cooperative boundaries, preserves completed work, emits one retained `run_stopped` terminal receipt, and promotes retries from the synchronous acknowledgement to that terminal receipt; the Python engine held wire-schema parity in that slice (Python runtime support landed in 0.13.0) (AC-894).
...
- AC-885: evaluator-epoch identity for LLM-judge scores. Judge results carry a content-addressed
...
- AC-799 exploration collapse guard detects advisory regretful-teacher/hint collapse signals across response length, diversity, entropy, route repetition, rollback, and score movement with opt-in mitigation.
