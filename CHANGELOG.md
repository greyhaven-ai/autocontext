...
- AC-756: `ImprovementResult.met_threshold` now consistently reports `best_score >= quality_threshold`. Previously the fallthrough exit (plateau-stall, unchanged-output, max-rounds, consecutive-failures) hard-coded `met_threshold=False`, so a run that produced compiling, above-threshold output via, e.g., a plateau-stall path was flagged as "didn't meet threshold" and could be discarded by automation. Early-return exits already used the same semantics so this only changes the fallthrough behavior.
