...
- AC-725: `docs/flue-influences.md` design note records what the runtime workspace/session contract, scoped command/tool grants, child-agent task execution, and `cwd` discovery model borrowed from a Flue review, and what was explicitly NOT borrowed (Flue dependency, API names, provider stack, vocabulary replacement). Cross-linked from `docs/README.md` "Architecture And Parity" and `docs/concept-model.md` "External Influences". Pins the guardrail that `sandbox` / `workspace` / `session` are runtime isolation/boundary concepts, not peer top-level product nouns alongside `Scenario` / `Mission`.
