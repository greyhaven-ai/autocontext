...
class TrainingRunner:
    ...
    def _init_git_repo(self) -> None:
        ...
        if not git_dir.exists():
            ...
            subprocess.run(
                ["git", "config", "user.email", "mts-train@local"],
                cwd=self.work_dir,
                capture_output=True,
                check=True,
            )
            subprocess.run(
                ["git", "config", "user.name", "MTS Training"],
                cwd=self.work_dir,
                capture_output=True,
                check=True,
            )
        ...
        subprocess.run(
            ["git", "commit", "-m", f"mts-train: setup workspace for {self.config.scenario}"],
            cwd=self.work_dir,
            capture_output=True,
            check=True,
        )
        ...
        branch_name = f"mts-train/{self.config.scenario}/{timestamp}"
        subprocess.run(
