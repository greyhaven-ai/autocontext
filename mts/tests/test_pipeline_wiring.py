...
from mts.knowledge.protocol import ResearchProtocol
from mts.knowledge.rapid_gate import RapidGateResult
...
class TestTuningLoadAtStartup:
    ...
    def test_tuning_loaded_when_config_adaptive_enabled(self) -> None:
        ...
        from mts.loop.stages import stage_knowledge_setup
        ...
        with patch("mts.loop.stages.build_prompt_bundle", return_value=_mock_prompt_bundle()):
...
class TestRapidGateInTournament:
    ...
    def test_rapid_gate_used_when_exploration_mode_rapid(self) -> None:
        ...
        with patch("mts.loop.stages.EvaluationRunner") as MockRunner, \
             patch("mts.loop.stages.ScenarioEvaluator"), \
             patch("mts.loop.stages.rapid_gate") as mock_rapid_gate:
...
class TestRapidToLinearTransition:
    ...
    def test_transition_after_rapid_gens(self) -> None:
        ...
        with patch("mts.loop.stages.EvaluationRunner") as MockRunner, \
             patch("mts.loop.stages.ScenarioEvaluator"), \
             patch("mts.loop.stages.rapid_gate") as mock_rapid_gate, \
             patch("mts.loop.stages.should_transition_to_linear") as mock_transition:
            MockRunner.return_value.run.return_value = mock_tournament
            mock_rapid_gate.return_value = RapidGateResult(
                decision="advance", delta=0.1, reason="improved",
            )
...
class TestTuningFromArchitect:
    ...
    def test_tuning_proposal_parsed_when_config_adaptive_enabled(self) -> None:
        ...
        from mts.loop.stages import stage_agent_generation
        ...
        with patch("mts.loop.stages.parse_dag_changes", return_value=[]):
