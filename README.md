...
- **Training pipeline work** adds opt-in teacher-reasoning distillation, sampling controls, MLX-LM fine-tuning, score-conditioned generation, reward-weighted loss, self-improving local-training loops, an R1-style distill-then-RLVR pipeline, online RLVR (GRPO/GSPO), on-policy distillation, and a cross-platform TRL backend (GKD + GRPO). These features are on `main` and in [CHANGELOG.md](CHANGELOG.md), but they are not included in the pinned `0.6.0` packages yet.
- **Validated result:** on GSM8K, the cross-platform TRL backend reproduced on-policy distillation end to end (a 1.5B student distilled from a 3B teacher closed ~30-43% of the gap, while RLVR stayed flat at matched compute), driven by autocontext's own scenario and verifier abstraction. Write-up: [autocontext/docs/case-study-on-policy-distillation.md](autocontext/docs/case-study-on-policy-distillation.md).
